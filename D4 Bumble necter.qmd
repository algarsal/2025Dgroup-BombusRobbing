...
```{r}
library(MASS)
mod_p <- glm(native_visits ~ rob_count + Population, family = poisson(link = "log"), data = df)

# use df.residual(mod_p) to get model df (NOT df.residual[1])
...
#3 Nectar: pierced vs undamaged (paired)
```{r}
# use Standing crop columns if available
...
m_seed_p <- glm(seeds_pr ~ rob_count + Population, family = poisson, data = df)
disp_s <- sum(residuals(m_seed_p, type = "pearson")^2, na.rm=TRUE) / m_seed_p$df.residual
cat("Seeds model dispersion:", round(disp_s, 2), "\n")
if (disp_s > 1.5) {
mn <- try(MASS::glm.nb(seeds_pr ~ rob_count + Population, data = df), silent = TRUE)
